...
public:
	void execute(std::string command) {
		PlaySound(TEXT("Resources/soundofwave.wav"), NULL, SND_SYNC);
	}
};
...
int main()
{
		...
		glfwWindowShouldClose(window.getWindow()) == 0)
	{
		...
		lightPos.y = 360.0f * sin(glfwGetTime() * 0.25f);
		lightPos.z = cos(glfwGetTime() * 0.25f) * 360.0f;
		...
		glUniform3f(glGetUniformLocation(shader.getId(), "lightColor"), lightColor.x, lightPos.y, lightColor.z);
		...
		if (!(lightPos.y < 0)) {
			...
			glUniform3f(glGetUniformLocation(terrainShader.getId(), "lightColor"), 0, 0, 0);
			glUniform3f(glGetUniformLocation(terrainShader.getId(), "lightPos"), 0, 0, 0);
		}
		...
		glUniform1f(glGetUniformLocation(terrainShader.getId(), "time"), currentFrame);
		glUniform1f(glGetUniformLocation(terrainShader.getId(), "amplitude"), 3.0f);
		glUniform2f(glGetUniformLocation(terrainShader.getId(), "dir"), 20.0f, 25.0f);
		glUniform2f(glGetUniformLocation(terrainShader.getId(), "dirr"), 10.0f, 50.0f);
		float centerX = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / 1));
		float centerY = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / 1));
		glUniform2f(glGetUniformLocation(terrainShader.getId(), "center"), centerX, centerY);
		glUniform1f(glGetUniformLocation(terrainShader.getId(), "length"), 2000.0f);
		glUniform1f(glGetUniformLocation(terrainShader.getId(), "speed"), 1600.0f);
		...
		if (!(lightPos.y < 0)) {
			...
			glUniform3f(glGetUniformLocation(shader.getId(), "lightColor"), 0, 0, 0);
			glUniform3f(glGetUniformLocation(shader.getId(), "lightPos"), 0, 0, 0);
		}

		ModelMatrix = glm::mat4(1.0);
		ModelMatrix = glm::translate(ModelMatrix, glm::vec3(10.0f, -5.0f, 15.0f));
